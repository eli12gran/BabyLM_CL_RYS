...
TRAINING_CONFIG = {
    ...
    # Model capacity, independent of the 128-token training blocks
    "n_positions": 1024,
    "n_ctx": 1024,
    ...
    "output_dir": "./model_cl_dale_chall",
    "babylm_checkpoint_dir": "./babylm_checkpoints_cl_dale_chall",
    "detailed_checkpoint_dir": "./checkpoints_detailed_cl_dale_chall",
...
# Your HF dataset with complexity metrics
HF_DATASET       = "eligran12/babylm_complexity_metrics"
COMPLEXITY_COL   = "complexity_dale_chall"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train GPT-2 with Curriculum Learning ordered by Dale-Chall (easy→hard)."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Drop rows where complexity_dale_chall is null (default: True).",
    )
...
def load_and_prepare_dataset(
    ...
    complexity_dale_chall (easy → hard), then tokenise and chunk.
...
class WordExposureCheckpointCallback(TrainerCallback):
    """
    Saves a full model checkpoint whenever cumulative word exposure crosses
    each threshold in checkpoint_intervals.  Identical to the surrogate version.
    """
