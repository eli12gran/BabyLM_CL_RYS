#!/usr/bin/env python
# NOTE: Paste this in strict/evaluation_pipeline/AoA_word/evaluation_functions_modified.py
import logging
import random
import typing as t
...
class StepSurprisalExtractor:
    ...
    def compute_surprisal_causal(
        ...
        # 1. Retrieve max position embeddings from the model configuration
        max_positions = (
            getattr(model.config, "n_positions", None)
            or getattr(model.config, "max_position_embeddings", None)
        )

        # 2. Left-truncate aligned tensors if the sequence length exceeds the context window
        if max_positions is not None and input_tokens.shape[1] > max_positions:
            input_tokens = input_tokens[:, -max_positions:]
            target_tokens = target_tokens[:, -max_positions:]
            attn_mask = attn_mask[:, -max_positions:]
