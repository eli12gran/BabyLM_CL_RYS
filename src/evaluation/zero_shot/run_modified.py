...
# NOTE: Put this one on strict/evaluation_pipeline
# ----------------------
...
from evaluation_pipeline.sentence_zero_shot.compute_results_modified import compute_results
...
def main():
    ...
    # Initialize variables so the `finally` block doesn't throw a ReferenceError
    results, predictions = {}, {}
    best_temp = -1
    ...
    try:
        ...
        traceback.print_exc()
        print("Attempting to salvage predictions data...")

    finally:
        # Save predictions whether the script succeeded or crashed
        if args.save_predictions and predictions:
            if best_temp != -1 and best_temp in predictions:
                # Standard save if we found the best temperature
                save_predictions(args, predictions, best_temp)
                print(f"Successfully saved predictions for task: : {args.task}, {args.data_path.stem}")
            else:
                # Emergency dump of all state if we crashed before finding best_temp
                emergency_path = args.output_path / "emergency_predictions_dump.json"
                with emergency_path.open("w") as f:
                    json.dump(predictions, f, indent=4)
                print(f"Emergency save complete. Raw predictions dumped to: {emergency_path}")
