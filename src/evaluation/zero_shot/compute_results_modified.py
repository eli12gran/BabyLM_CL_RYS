...
# NOTE: Put this one on strict/evaluation_pipeline/sentence_zero_shot
# ------------------------
...
def compute_results(args: argparse.ArgumenParser, model: torch.nn.Module, dataloader: DataLoader, temperatures: list[float]):
...
def _live_save_predictions(args, temperatures, predictions):
    """Helper function to format and live-save current predictions to disk.
    """
    ...
    try:
        ...
        for i in temperatures:
            ...
            live_predictions[i] = temp_pred
            
        live_save_path = args.output_path / "live_predictions.json"
        with live_save_path.open("w") as f:
            json.dump(live_predictions, f, indent=4)
    except Exception as e:
        print(f"\n[WARNING] Failed to write live predictions to disk: {e}")
    ...
    for raw_sentences, sentence_dict, labels, metadatas, uids, images in tqdm(dataloader):
        try:
            ...
            for prefix in prefixes:
                input_ids = sentence_dict[f"{prefix}_inputs"]
                attention_mask = sentence_dict[f"{prefix}_attn_mask"]
                targets = sentence_dict[f"{prefix}_targets"]
                phrase_mask = sentence_dict[f"{prefix}_phrase_mask"]

                # Some BabyLM examples can tokenize beyond small GPT-style context
                # windows. For 256-position models, left-truncate all aligned tensors
                # so the evaluated phrase stays aligned with the final model positions.
                if max_positions is not None and input_ids.shape[1] > max_positions:
                    print(
                        f"[TRUNCATE] task={args.task} prefix={prefix}: "
                        f"seq_len={input_ids.shape[1]} > max_positions={max_positions}"
                    )
                    input_ids = input_ids[:, -max_positions:]
                    attention_mask = attention_mask[:, -max_positions:]
                    targets = targets[:, -max_positions:]
                    phrase_mask = phrase_mask[:, -max_positions:]
                if no_image:
                    logits = model(
                ...
                if logits.size(1) != input_ids.size(1):  # Assumption is that images are prepended to the text when done post-tokenization.
            ...
            _live_save_predictions(args, temperatures, predictions)
            
        except Exception as e:
            print(f"\n[ERROR] Skipping batch due to error in causal evaluation loop: {e}")
            traceback.print_exc()
...
def compute_mlm_results(args, model, dataloader, temperatures):
    ...
    for raw_sentences, sentence_dict, labels, metadatas, uids, images in tqdm(dataloader):
        try:
            ...
            print(f"\n[ERROR] Skipping batch due to error in MLM evaluation loop: {e}")
            traceback.print_exc()
...
def compute_enc_dec_mask_results(args, model, dataloader, temperatures):
    ...
    for raw_sentences, sentence_dict, labels, metadatas, uids, images in tqdm(dataloader):
        try:
            ...
            print(f"\n[ERROR] Skipping batch due to error in encoder-decoder mask loop: {e}")
            traceback.print_exc()
...
def compute_enc_dec_prefix_results(args, model, dataloader, temperatures):
    ...
    for raw_sentences, sentence_dict, labels, metadatas, uids, images in tqdm(dataloader):
        try:
            ...
            print(f"\n[ERROR] Skipping batch due to error in encoder-decoder prefix loop: {e}")
            traceback.print_exc()
