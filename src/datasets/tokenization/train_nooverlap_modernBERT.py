...
from datasets.tokenization.tokenizer_utilities_nooverlap import NOOVERLAPTokenizer, SPECIAL_TOKENS
    ...
    for lang, hf_path in HF_DATASETS.items():
        ...
        for idx, example in enumerate(raw_dataset):
            ...
            n_tokens = count_official_tokens(
                text, lang, zho_counter_tokenizer=zho_counter_tokenizer,
            )
            if n_tokens <= 0:
                continue
...
class NOOVERLAPMLMCollator:
    ...
    Standard BERT-style masking:
      - 15% of non-special, non-padding positions selected
      - of those: 80% -> <MASK>, 10% -> random token, 10% -> original
      - labels are original IDs at masked positions, -100 elsewhere
    """
    ...
    def __call__(self, batch):
        ...
        indices_replaced = (
            torch.bernoulli(replace_prob, generator=self.generator).bool() & masked_indices
        )
        ...
        random_words = torch.randint(
            low=0, high=self.vocab_size, size=labels.shape,
...
def setup_training(
    ...
    # Trainer is instantiated here — not via a recursive call to setup_training.
    trainer = Trainer(
        model=model,
        args=training_args,
        ...
        json.dump(
            {
                ...
                "auto_map": {
                    "AutoTokenizer": ["tokenization_nooverlap.NOOVERLAPHFTokenizer", None]
                },
                "trust_remote_code": True,
                "strategy": "NOOVERLAP",
